...
function SearchComponent(props) {
    ...
    function addToDestinations(map){
        var lat=search.selected_lat_lng.lat;
        var lng=search.selected_lat_lng.lng;
    ...
    function showInMap() {
        console.log(search.selected_lat_lng);
        moveMapToPlace(map,search.selected_lat_lng.lat, search.selected_lat_lng.lng);
        setSearch({
            ...search,
            reply_places:[]
        });
    ...
    function searchApi(){
        ...
        .then(data=>{
            ...
            data["items"].forEach(place=>{
                onchange_radio.push(<PlaceSearchButton key={`place-${n}`} radio_name="places_result" state={search} setSearch={setSearch} lat={place.position.lat} lng={place.position.lng} place={place}/>);
                n++;
            })
            places.push(
            <>
            ...
            <div class="row gx-5 mt-2">
                <div className="col d-flex justify-content-center">
                <button class="btn btn-warning" onClick={showInMap}>Mostrar en el mapa</button>
                </div>
                <div className="col d-flex justify-content-center">
                <button class="btn btn-primary" onClick={()=>addToDestinations(map)}>Agregar parada</button>
                </div>
            </div>
            </>
            );
    ...
    return(
        ...
        <div class="shift_started_modal" style={{display:search.reply_places.length<=0?"none":"block"}}>
...
function PlaceSearchButton(props) {
    ...
    if(props.state.selected_lat_lng){
        color=props.state.selected_lat_lng.lat==parseFloat(props.lat)&&props.state.selected_lat_lng.lng==parseFloat(props.lng)?"btn-primary":"btn-secondary"
    }
    function onChangeRadio() {
        console.log({"lat":parseFloat(props.lat),"lng":parseFloat(props.lng)});
        props.setSearch(prevState => ({ ...prevState, selected_lat_lng:{"lat":parseFloat(props.lat),"lng":parseFloat(props.lng)}}))
    }
    return(
        <label className={`btn btn-block ${color}`}>
            {props.place.address.label}
            <input type="radio" style={{display:"none"}} onChange={onChangeRadio} value={[props.lat,props.lng]} name={props.radio_name}></input>
        </label>
    )
}
...
function DestinationsModal(props) {
    ...
    const destinations= ()=>{
        ...
        for (let index = 0; index < props.state.destinations; index++) {
            ...
            <div style={{display:"inline-block"}}>
                ...
                <button className='btn btn-light'><i class="fa-solid fa-grip text-secondary"></i></button>
                <button class="btn btn-block btn-secondary">{props.state.destinations[index].name}</button>
            </div>
            );
        }
        return destinations;
    ...
    return (
        <div>
            ...
            <button type="button" class="btn-block btn-primary btn" onClick={()=>openModal(false)}>Listo!</button>
        </div>
    )
}
...
function TransportationModal(props) {
    ...
    return(
            <a>
                <input type="radio" onChange={updateTransportation} class="transportation" value="car" name="transportation" id=""/><i class="fas fa-car transportation-icon"></i>
                <input type="radio" onChange={updateTransportation} class="transportation" value="truck" name="transportation" id=""/><i class="fas fa-truck transportation-icon"></i>
                <input type="radio" onChange={updateTransportation} class="transportation" value="pedestrian" name="transportation" id=""/><i class="fas fa-walking transportation-icon"></i>
                <input type="radio" onChange={updateTransportation} class="transportation" value="bicycle" name="transportation" id=""/><i class="fas fa-bicycle transportation-icon"></i>
            </a>
    )
}
...
function ModeModal(props) {
    return(
        <a>
            ...
            <input type="radio" checked class="mode" value="none" name="mode" id=""/><span>Ninguno</span>
            <input type="radio" class="mode" value="fastest" name="mode" id=""/><span>Rápida</span>
            <input type="radio" class="mode" value="shortest" name="mode" id=""/><span>Corta</span>
        </a>
    )
}
...
function AvoidModal(params) {
    return (
        <a>
            <div>
                <input type="checkbox" class="avoid" value="seasonalClosure" name="avoid" id="">
                    <span>Clausura estacional</span>
                </input>
            </div>
            <div>
                <input type="checkbox" class="avoid" value="controlledAccessHighway" name="avoid" id="">
                    <span>Autopista de acceso controlado</span>
                </input>
            </div>
            <div>
                <input type="checkbox" class="avoid" value="carShuttleTrain" name="avoid" id="">
                    <span>Cruce con tren</span>
                </input>
            </div>
            <div>
                <input type="checkbox" class="avoid" value="dirtRoad" name="avoid" id="">
                    <span>Camino de tierra</span>
                </input>
            </div>
            <div>
                <input type="checkbox" class="avoid" value="uTurns" name="avoid" id="">
                    <span>Vuelta en U</span>
                </input>
            </div>
            <div>
                <input type="checkbox" class="avoid" value="difficultTurns" name="avoid" id="">
                    <span>Carretera de cobro</span>
                </input>
            </div>
            <div>
                <input type="checkbox" name="avoid" value="ferry"  class="avoid" id="">
                    <span>Ferry</span>
                </input>
            </div>
            <div>
                <input type="checkbox" name="avoid" value="tunnel" class="avoid" id="">
                    <span>Túnel</span>
                </input>
            </div>
            <div>
                <input type="checkbox" name="avoid" value="tollRoad" class="avoid" id="">
